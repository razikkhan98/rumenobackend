...
exports.transferAnimal = expressAsyncHandler(async (req, res) => {
  ...
  try {
    const { tagId, uniqueId, uid, date, animalStatus } = req.body;
    ...
    const existingTransferAnimal = await TransferAnimal.findOne({
      tagId,
      uniqueId,
      uid,
    });
...
exports.getTransferAnimalByUniqueId = expressAsyncHandler(async (req, res) => {
  ...
  try {
    ...
    const animal = await TransferAnimal.findOne({ uid });
